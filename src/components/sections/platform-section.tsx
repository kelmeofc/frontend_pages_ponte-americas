            ...
            <ul className="space-y-3 mb-6">
              {features.map((item) => (
                <li key={item.label} className="flex items-center gap-3">
                  <span className="w-8 h-8 p-2 bg-indigo-50 text-indigo-600 rounded-full shrink-0 flex items-center justify-center" aria-hidden="true">
                    {item.icon}
                  </span>
                  <span className="text-gray-800 text-base font-medium font-['Rubik'] leading-5">{item.label}</span>
                </li>
              ))}
            </ul>
